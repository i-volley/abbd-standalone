...
@section('content')
<h2 class="mb-4">{{ __('Nuovo Microciclo') }}</h2>
<form action="{{ route('allenatore.macrocicli.microcicli.store', $macrociclo) }}" method="POST">
    ...
    <div class="row g-3" style="max-width:400px">
        ...
        <div class="col-6">
            <label class="form-label">{{ __('Data inizio (lunedì)') }}</label>
        ...
        <div class="col-12">
            ...
            <a href="{{ route('allenatore.macrocicli.show', $macrociclo) }}" class="btn btn-outline-secondary ms-2">{{ __('Annulla') }}</a>
        </div>
    </div>
</form>
@endsection
