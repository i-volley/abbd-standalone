...
<div class="card shadow-sm border-0 mb-4">
    <div class="card-body py-2">
        <small class="text-muted fw-semibold text-uppercase" style="font-size:.7rem;letter-spacing:.07em">{{ __('Progressione attesa') }}</small>
        <div class="d-flex gap-2 mt-2 flex-wrap align-items-center">
            @foreach($sequenza as $i => $metod)
                <div class="text-center">
                    <div class="badge {{ $metodBadge[$metod] ?? 'bg-secondary' }} px-3 py-2" style="font-size:.85rem">
                        {{ __('Seduta') }} {{ $i + 1 }}<br><span style="font-size:.7rem">{{ strtoupper($metod) }}</span>
                    </div>
                </div>
                @if(!$loop->last)
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    ...
                    @if(!$inLinea)
                        <span class="badge bg-warning text-dark rounded-pill" style="font-size:.65rem" title="Metodologia attesa: {{ $metodologiePrev }}">
                            ⚠️ {{ __('Non in linea con progressione') }}
                        </span>
                    @endif
                </div>
...
@empty
<p class="text-muted">{{ __('Nessuna seduta ancora.') }} <a href="{{ route('allenatore.sedute.create') }}?unita_didattica_id={{ $unitaDidattica->id }}">{{ __('Aggiungi la prima') }}</a>.</p>
@endforelse
